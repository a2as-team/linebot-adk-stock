...
- Multi-agent architecture:
  - A general agent for creative plan suggestions (e.g., dating plans) using Google Search. (Note: This part seems to be a planned feature, the current `main.py` only defines `stock_agent`)
  - A specialized agent (`stock_agent`) for stock market queries (prices, performance, changes).
- Support for function calling with custom tools for the `stock_agent`.
- Integration with LINE Messaging API.
- Built with FastAPI for high-performance async processing.
- Containerized with Docker for easy deployment.
- In-memory session management for user interactions.
...
The bot currently utilizes a single agent setup focused on stock information:

1. **Stock Agent (`stock_agent`):**
   - **Description:** Provides stock market information and analysis. It is defined in `main.py` and uses tools from `multi_tool_agent/stock_agent.py`.
   - **Tools (from `multi_tool_agent/stock_agent.py`):**
     - `get_stock_price`: Fetches the current or most recent closing price of a given stock symbol.
   ...
   - **Session Management:** `main.py` includes an in-memory session service (`InMemorySessionService`) to manage user sessions, creating a new session for a user if one doesn't exist or reusing an existing one.

(Note: The "Location Search Agent (`root_agent`)" mentioned previously seems to be a placeholder or future plan, as the current `main.py` initializes `stock_agent` as the `root_agent` for the `Runner`.)
